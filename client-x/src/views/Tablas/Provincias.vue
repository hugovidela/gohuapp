      ...
      <v-data-table
        ...
        <template v-slot:top>
          <v-toolbar flat :color="colorSucursal">
            ...
            <v-toolbar-title class="body-1 white--text">Provincias</v-toolbar-title>
                    ...
                    <v-container>
                      ...
                      <v-row>
                        <v-col cols="12" sx="12" mx="12">
                          <v-autocomplete
                            class="body-2"
                            v-model="editado.pais_id"
                            :items="itemsPaises"
                            :loading="isLoadingPaises"
                            :search-input.sync="searchPaises"
                            item-text="nombre"
                            item-value="id"
                            label="Pais"
...
export default {
  ...
  computed: {
    ...
    itemsPaises () {
      return this.entriesPaises.map(entry => {
  ...
  watch: {
    ...
    searchPaises (val) {
      ...
      if (this.isLoadingPaises) return
      this.isLoadingPaises = true
      // Lazily load input items
      return HTTP().get('/paises')
        .then(({ data }) => {
          this.entriesPaises = data;
        })
        .catch(err => {
          console.log(err)
        })
        .finally(() => (this.isLoadingPaises = false))
    },
  },
  mounted () {
 ...
 methods: {
   ...
   closeForm(){
    ...
    guardar() {
      ...
      this.pais_id = this.editado.pais_id;
