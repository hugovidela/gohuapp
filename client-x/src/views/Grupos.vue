    ...
    <v-flex>
      ...
      <div class="col-md-12 mx-auto">
        <table class="table table-hover">
          <thead>
            <tr>
                <th class="text-left">Path: {{pathComputed}}</th>
            </tr>
          </thead>
          <tbody>
            <tr>
                ...
                <div slot-scope="{ node }" class="node-container">
                  <table class="table table-hover">
                    <template>
                      ...
                      <v-btn
                        class="mr-2" fab dark x-small color="red"
...
<script>
  ...
  data() {
    ...
    methods: {
      ...
      armarPath (val) {
        return HTTP().get(`/grupos/exists/${val}`)
          .then(({ data }) => {
            if (data.grupo_id !== 0) {
              this.armarPath(data.grupo_id)
            }
        ...
        return HTTP().get('/grupos').then(response => {
          ...
          for(var i in response.data) {
            g.push({
                ...
                padre: response.data[i].grupo_id,
      ...
      activarDesactivar(node, e) {
        ...
        if (node.children.length>0) {
          this.msg.msgBody = `No puede desacrivar Items que poseean items anidados.`
          this.msg.msgTitle = 'Información'
          this.msg.msgAccion = 'informacion'
          this.msg.msgButtons = ['Aceptar']
          this.msg.msgVisible = true
        } else {
          const valor = node.states.disabled ? 1 : 0;
          node.states.disabled = !node.states.disabled;
          HTTP().patch(`grupos/${node.id}`,{activo: valor}).then ((res) => {
              console.log(res);
            }).catch((err)=>{
              console.log(err);
            })
      ...
      mostrarInfo(node) {
        let data = HTTP().get(`/grupos/exists/${node.id}`)
          .then(({ data }) => {
      ...
      addChildNode(node) {
        if (node.enabled()) {
          node.append('Nueva caracteristica')
        }
      },
      initEventViewer(event) {
        ...
        return function (node, newNode) {
          ...
          if(event.name === "node:text:changed"){
            ...
            HTTP().post("/grupos", {nombre: 'Nuevo Grupo', grupo_id: padre, activo: 1}).then(() => {
