...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'NOMBRE', value:'nombre', sortable: false, width: 600},
      { text: 'GRUPO', value:'nomgru', sortable: false, width: 150},
      { text: 'MARCA', value:'nommar', sortable: false, width: 150},
...
//        this.tagsDisp = data;
      ...
      const c = HTTP().get('/marcasbus')
        .then(({ data }) => {
          this.marItems = [];
          data.forEach(element => {
            this.marItems.push(element)
          })
        });
      const d = HTTP().get('/ivabus')
        .then(({ data }) => {
          this.ivaItems = [];
          debugger
          data.forEach(element => {
            this.ivaItems.push(element)
          })
        });
        */
...
//          this.items.push(data[i].articulo)
    ...
    */
    ...
    getJsonData () {
      ...
      // debugger
      return HTTP().get(`userarticulos/${this.pagination.page}/9/${s}`)
                  ...
                  .then(({ data }) => {
                    ...
                    if (data.length > 0) {
                      this.editado.precios.push({
                        ...
                        precioFinal: data[0].precio*(1+(this.editado.iva.tasa/100)),
